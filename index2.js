...
const students = [
 ...
 function getSubjectsStatistics(students) {
  let averagee=students.reduce((total,value)=>(total+value.grades)/3);
  return students.map(student=> {name:student.firstName,averagee:average}
 )
 }
 
 function getHighAchievers(students, gradeThreshold) {
   let aver=students.reduce((total,value)=>(total+value.grades)/3);
   return [...students]
   .filter(studentGrade=>studentGrade.aver>gradeThreshold)
   .map(student=>({name:student.name,lastName:student.lastName}))
 }
 ...
 //function getMultiAchivers(students, threshold) {
   ...
   return students.map(student=>({
      student.filter(stud=>stud.grades>threshold),
      {name:student.name,}
   })
   )
